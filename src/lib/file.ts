...
export const writeFile = (filePath: string, data: any, append = true): boolean => {
  if (mkdir(filePath)) {
...
export const fileExist = (path: string): boolean => fs.existsSync(path)

export const removeFiles = (path: string): boolean => {
  if (fileExist(path)) fs.rmSync(path)
...
export const fileExistAsync = (path: string) => asyncExec(() => fileExist(path))

export const removeFilesAsync = (path: string) => asyncExec(() => removeFiles(path))

export const computedHash = (buffer: Buffer, Hash?: string): string =>
  crypto.createHash(Hash ?? 'md5').update(buffer).digest('hex')
